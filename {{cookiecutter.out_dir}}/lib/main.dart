...
# fix for: https://github.com/fui-dev/serious-python/issues/85#issuecomment-2065000974
os.environ["OPENBLAS_NUM_THREADS"] = "1"

if os.getenv("fui_PLATFORM") == "android":
    import ssl
...
callback_socket_addr = os.getenv("fui_PYTHON_CALLBACK_SOCKET_ADDR")
...
Future prepareApp() async {
  if (kIsWeb) {
    ...
    if (defaultTargetPlatform != TargetPlatform.iOS &&
      ...
      appDataPath = path.join(appDataPath, "fui", packageInfo.packageName);
      if (!await Directory(appDataPath).exists()) {
        await Directory(appDataPath).create(recursive: true);
      }
    }

    environmentVariables["fui_APP_STORAGE_DATA"] = appDataPath;
    environmentVariables["fui_APP_STORAGE_TEMP"] = appTempPath;

    outLogFilename = path.join(appTempPath, "console.log");
    environmentVariables["fui_APP_CONSOLE"] = outLogFilename;

    environmentVariables["fui_PLATFORM"] =
    ...
    if (defaultTargetPlatform == TargetPlatform.windows) {
      ...
      environmentVariables["fui_SERVER_PORT"] = tcpPort.toString();
    } else {
      // use UDS on other platforms
      pageUrl = "fui_$pid.sock";
      environmentVariables["fui_SERVER_UDS_PATH"] = pageUrl;
    }
  }
...
Future<String?> runPythonApp(List<String> args) async {
  ...
  environmentVariables["fui_PYTHON_CALLBACK_SOCKET_ADDR"] = socketAddr;
...
Future setupDesktop() async {
  if (isDesktop()) {
    ...
    var hideAppOnStart = env["fui_HIDE_APP_ON_START"];
